...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--surf-dir-template", default=None, help="Optional subject surface directory template, e.g. /path/sub-{subject}/hippunfold/sub-{subject}/surf")
...
def absolutize_scene_paths(root: ET.Element, scene_path: Path, surf_dir: Path | None = None) -> None:
    ...
    for obj in root.iter("Object"):
        if obj.attrib.get("Type") == "pathName" and obj.text:
            ...
            if surf_dir is not None:
                surf_match = surf_dir / candidate.name
                if surf_match.exists():
                    obj.text = str(surf_match)
                    continue
...
def main() -> int:
    ...
    for subject in args.subjects:
        ...
        surf_dir = Path(args.surf_dir_template.format(subject=subject)).resolve() if args.surf_dir_template else None
        absolutize_scene_paths(root, scene_path, surf_dir=surf_dir)
