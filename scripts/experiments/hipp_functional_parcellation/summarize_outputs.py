...
from run_subject import REPO_ROOT, render_locked_grid_png, save_combined_label_assets
...
SMOOTH_COLORS = {"light": "#f58518", "4mm": "#54a24b"}
SOFT_DIAG_COLORS = {
    ("light", "mean_probabilities"): "#4c78a8",
    ("light", "mean_regularized_probabilities"): "#f58518",
...
def plot_soft_diagnostics(ax: plt.Axes, final_selection: dict[str, object], hemi: str) -> None:
    color_specs = [
        ("light", "mean_probabilities", "1-ring mean"),
        ("light", "mean_regularized_probabilities", "1-ring reg"),
        ("4mm", "mean_probabilities", "4mm mean"),
        ("4mm", "mean_regularized_probabilities", "4mm reg"),
    ]
    networks = list(final_selection["per_smooth"]["light"]["hemis"][hemi]["soft_outputs"]["networks"])
...
def union_top_network_indices(probs: np.ndarray, top_n_per_cluster: int = 3) -> np.ndarray:
    keep: set[int] = set()
    n_networks = probs.shape[1]
    top_n = min(top_n_per_cluster, n_networks)
    for row in probs:
        order = np.argsort(row)[::-1]
        keep.update(int(idx) for idx in order[:top_n])
    keep_list = sorted(keep, key=lambda idx: float(np.nanmax(probs[:, idx])), reverse=True)
    return np.asarray(keep_list, dtype=np.int32)
...
def create_probability_figure(root: Path, final_selection: dict[str, object]) -> Path:
    ...
    axs = [fig.add_subplot(gs[0, idx]) for idx in range(2)]
    ...
    for ax, hemi in zip(axs, HEMIS, strict=True):
        if branch_slug == "prob-soft":
            ...
            for smooth_name in SMOOTHS:
                ...
                row_labels.extend(
                    [f"{SMOOTH_LABEL[smooth_name]} {row['cluster_name']}" for row in hemi_node["cluster_annotations"]]
                )
            probs = np.asarray(block_rows, dtype=np.float32)
            keep = union_top_network_indices(probs, top_n_per_cluster=4)
        else:
            networks = list(final_selection["per_smooth"]["light"]["hemis"][hemi]["profile_networks"])
            block_rows = []
            row_labels = []
            for smooth_name in SMOOTHS:
                hemi_node = final_selection["per_smooth"][smooth_name]["hemis"][hemi]
            ...
            probs = np.asarray(block_rows, dtype=np.float32)
            keep = union_top_network_indices(probs, top_n_per_cluster=3)
        ...
        last_image = image
        title = f"{hemi} {'soft summaries' if branch_slug == 'prob-soft' else 'cluster profiles'}"
        ...
        summaries[hemi] = {
            ...
            "mode": "soft+subregions" if branch_slug == "prob-soft" else "cluster",
            "display_networks_union_top3": networks_plot,
            "display_probabilities_union_top3": probs_plot.tolist(),
    ...
    fig.subplots_adjust(left=0.10, right=0.97, bottom=0.20, top=0.86, wspace=0.42)
    out_path = root / "cluster_probability_heatmaps.png"
    fig.savefig(out_path, dpi=220, facecolor="white")
    plt.close(fig)
        ...
        for k in RENDER_SHORTLIST:
            ...
            if not (reuse_existing and render_png.exists()):
                ...
                assets = save_combined_label_assets(
                    ...
                    stem=f"hipp_cluster_{branch_slug.replace('-', '_')}_{smooth_name}_k{k}",
                )
...
def main() -> int:
    ...
    manifest = {
        ...
        "cluster_probability_heatmaps": str(probs),
