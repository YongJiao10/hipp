...
from sample_hipp_surface_timeseries import maybe_smooth
...
def write_surface_store_pointer(
    *,
    ...
    subject: str,
    light_left_path: Path,
    light_right_path: Path,
    fwhm_left_path: Path,
    ...
    payload = {
        ...
        "timeseries": {
            "light": {
                "left": str(light_left_path.resolve()),
                "right": str(light_right_path.resolve()),
            },
            "4mm": {
...
def write_reference_store_pointer(
    *,
    ...
    parcel_table_path: Path,
    parcel_timeseries_path: Path,
) -> None:
    pointer_dir.mkdir(parents=True, exist_ok=True)
    payload = {
        ...
        "artifacts": {
            ...
            "cortex_roi_parcels_tsv": str(parcel_table_path.resolve()),
            "cortex_roi_parcel_timeseries_npy": str(parcel_timeseries_path.resolve()),
        },
    }
...
def apply_retain_level(out_root: Path, retain_level: str) -> None:
    ...
    outputs_root = (REPO_ROOT / "outputs" / "hipp_functional_parcellation").resolve()
    out_root_resolved = out_root.resolve()
    try:
        out_rel = out_root_resolved.relative_to(outputs_root)
    except ValueError:
        out_rel = Path(out_root_resolved.name)
    archive_root = REPO_ROOT / "_archive" / "hipp_functional_parcellation" / "retain" / stamp / out_rel / retain_level
...
def group_fc_by_network(fc: np.ndarray, parcel_rows: list[dict[str, object]]) -> tuple[np.ndarray, list[str]]:
    networks = sorted({str(row["parent_network"]) for row in parcel_rows})
    grouped = []
    for network in networks:
        inds = [idx for idx, row in enumerate(parcel_rows) if row["parent_network"] == network]
        grouped.append(np.nanmean(fc[:, inds], axis=1))
    return np.asarray(grouped, dtype=np.float32).T, networks


def grouped_fc_to_probabilities(grouped_fc: np.ndarray) -> np.ndarray:
            ...
            rows.append(
                {
                    ...
                    "n_grayordinates": int(row["n_grayordinates"]),
                }
            )
    return rows
...
def run_gradient_branch(
    *,
    ...
    gradients, eigvals = diffusion_map_embedding(build_sparse_affinity(fc, 0.1), n_components=5)
    gradients_odd, eigvals_odd = diffusion_map_embedding(build_sparse_affinity(fc_odd, 0.1), n_components=5)
    gradients_even, eigvals_even = diffusion_map_embedding(build_sparse_affinity(fc_even, 0.1), n_components=5)
    ...
    np.save(feature_dir / "hipp_fc_gradients.npy", gradients.astype(np.float32))
    np.save(feature_dir / "hipp_fc_gradient_eigenvalues.npy", eigvals.astype(np.float32))
    np.save(feature_dir / "hipp_fc_gradients_odd.npy", gradients_odd.astype(np.float32))
    np.save(feature_dir / "hipp_fc_gradients_even.npy", gradients_even.astype(np.float32))
    save_json(
        feature_dir / "feature_summary.json",
        {
            "hemi": hemi,
            "feature_kind": "gradient",
...
def run_probability_branch(
    *,
    ...
    probabilities = grouped_fc_to_probabilities(grouped_fc)
    probabilities_odd = grouped_fc_to_probabilities(grouped_fc_odd)
    probabilities_even = grouped_fc_to_probabilities(grouped_fc_even)
...
def build_panel_titles(branch_slug: str, smooth_name: str, hemi: str, k_final: int) -> str:
    smooth_label = "1-ring" if smooth_name == "light" else smooth_name
    if branch_slug == "prob-soft":
        return f"{smooth_label} {hemi} probability-profile subregions (K={k_final})"
    if branch_slug == "wta":
        return f"{smooth_label} {hemi} winner-takes-all"
    return f"{smooth_label} {hemi} final K={k_final}"


def main() -> int:
    parser = argparse.ArgumentParser(description="Run single-subject hippocampal functional parcellation")
    parser.add_argument("--subject", default="100610")
    parser.add_argument("--branch", default="gradient", choices=BRANCHES)
    ...
    parser.add_argument("--out-root", default=str(REPO_ROOT / "outputs" / "hipp_functional_parcellation"))
    parser.add_argument(
    ...
    parcel_table_path = shared_reference_store_dir / "cortex_roi_parcels.tsv"
    parcel_timeseries_path = shared_reference_store_dir / "cortex_roi_parcel_timeseries.npy"
    reference_params = {
        "subject": subject,
        "atlas_slug": atlas_slug,
    ...
    write_reference_store_pointer(
        ...
        parcel_table_path=parcel_table_path,
        parcel_timeseries_path=parcel_timeseries_path,
    )

    reference_summary = json.loads(reference_summary_path.read_text(encoding="utf-8"))
    parcel_rows = load_reference_rows(parcel_table_path)
    parcel_ts = np.load(parcel_timeseries_path).astype(np.float32)
    ...
    light_left_path = shared_surface_store_dir / "light" / f"sub-{subject}_hemi-L_timeseries.npy"
    light_right_path = shared_surface_store_dir / "light" / f"sub-{subject}_hemi-R_timeseries.npy"
    ...
    surface_outputs = [light_left_path, light_right_path, fwhm_left_func, fwhm_right_func, fwhm_left_path, fwhm_right_path]
    if not stage_is_up_to_date(
        ...
        left_raw = load_metric_array(left_raw_metric, expected_n_vertices=int(left_coords.shape[0]))
        right_raw = load_metric_array(right_raw_metric, expected_n_vertices=int(right_coords.shape[0]))
        (shared_surface_store_dir / "light").mkdir(parents=True, exist_ok=True)
        np.save(light_left_path, sanitize_timeseries(maybe_smooth(left_raw, left_faces, 1).astype(np.float32)))
        np.save(light_right_path, sanitize_timeseries(maybe_smooth(right_raw, right_faces, 1).astype(np.float32)))

        for hemi, surface_path, metric_path in [
            ("L", left_surface, left_raw_metric),
            ("R", right_surface, right_raw_metric),
        ]:
            smooth_dir = shared_surface_store_dir / "4mm"
            smooth_dir.mkdir(parents=True, exist_ok=True)
            out_metric = smooth_dir / f"sub-{subject}_hemi-{hemi}_space-corobl_den-{hipp_density}_label-hipp_bold.func.gii"
            run(
                [
                    WB_COMMAND,
                    "-metric-smoothing",
    ...
    write_surface_store_pointer(
        ...
        subject=subject,
        light_left_path=light_left_path,
        light_right_path=light_right_path,
    ...
    smooth_specs: dict[str, dict[str, np.ndarray | str]] = {
        "light": {
            "left": np.load(light_left_path).astype(np.float32),
            "right": np.load(light_right_path).astype(np.float32),
            "description": "1-ring local smoothing",
        },
        "4mm": {
    ...
    compute_params = {
        ...
        "smoothings": SMOOTH_ORDER,
        "eval_k": EVAL_K,
        "strict_soft_route": bool(branch_slug == "prob-soft"),
    }
    compute_inputs = [
        parcel_table_path,
        parcel_timeseries_path,
    ...
    for smooth_name in SMOOTH_ORDER:
        compute_outputs.extend(
            [
                ...
                / smooth_name
                / "final"
                / f"sub-{subject}_hemi-L_space-corobl_den-{hipp_density}_label-hipp_cluster_{branch_tag}.label.gii",
                workbench_dir
                / smooth_name
                / "final"
                / f"sub-{subject}_hemi-R_space-corobl_den-{hipp_density}_label-hipp_cluster_{branch_tag}.label.gii",
                workbench_dir
                / smooth_name
                / "final"
                / f"sub-{subject}_space-corobl_den-{hipp_density}_label-hipp_cluster_{branch_tag}.dlabel.nii",
            ]
        )
    if not stage_is_up_to_date(
        ...
        for smooth_name in SMOOTH_ORDER:
            ...
            for hemi, ts, adj in [("L", left_clean, left_adj), ("R", right_clean, right_adj)]:
                ...
                fc = corrcoef_rows(ts, parcel_ts)
                np.save(hemi_fc_dir / "hipp_vertex_to_parcel_fc.npy", fc.astype(np.float32))
                save_json(
                    hemi_fc_dir / "fc_summary.json",
                    {
                        ...
                        "n_parcels_used": int(parcel_ts.shape[0]),
                    },
                )

                odd_idx = np.arange(ts.shape[1]) % 2 == 0
                even_idx = ~odd_idx
                fc_odd = corrcoef_rows(ts[:, odd_idx], parcel_ts[:, odd_idx])
                fc_even = corrcoef_rows(ts[:, even_idx], parcel_ts[:, even_idx])
                grouped_fc, networks = group_fc_by_network(fc, parcel_rows)
                grouped_fc_odd, _net_odd = group_fc_by_network(fc_odd, parcel_rows)
                grouped_fc_even, _net_even = group_fc_by_network(fc_even, parcel_rows)
                ...
                if branch_slug == "gradient":
                    cluster = run_gradient_branch(
                        fc=fc,
                        fc_odd=fc_odd,
                        fc_even=fc_even,
                        grouped_fc=grouped_fc,
                        networks=networks,
                        connectivity=adj,
                    ...
                    cluster = run_probability_branch(
                        grouped_fc=grouped_fc,
                        grouped_fc_odd=grouped_fc_odd,
                        grouped_fc_even=grouped_fc_even,
                        networks=networks,
                        connectivity=adj,
                        ...
                        soft_dir=soft_dir,
                        hemi=hemi,
                        save_soft_extras=(branch_slug == "prob-soft"),
                        strict_soft_route=(branch_slug == "prob-soft"),
                    )
            ...
            final_assets = save_combined_label_assets(
                ...
                stem=f"hipp_cluster_{branch_tag}",
                density=hipp_density,
            )
